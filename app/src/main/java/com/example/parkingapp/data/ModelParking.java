...
public class ModelParking {
    ...
    public ModelParking(Application application) {
    ...
    public void registerCar(String plateCar) {
        final Car car = new Car(plateCar);
        final CeibaDataBase db = CeibaDataBase.getDatabase(application);
        db.databaseWriteExecutor.execute(new Runnable() {
            @Override
            public void run() {
                int numberCar = db.parkingDao().getAllParkinList().get(0).getNumberCar();
                ...
                for (ParkingSpace item: parkingSpaceList){
                    if (item.isState()== false){
                        ...
                        car.setFkParkingSpace(item.getParkingSpaceId());
                        db.parkingSpaceDao().setUpdateStateParking(true, item.getParkingSpaceId());
                        break;
                    }
                }
