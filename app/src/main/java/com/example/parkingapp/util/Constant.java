package com.example.parkingapp.util;

public abstract class Constant {

    final public static String REGISTER_SUCCESSFULL = "Registro exitoso";
    final public static String REGISTER_UNSUCCEFULL = "Ocurrio un problema, intente nuevamente";
    final public static String COSTO_TOTAL = "Su costo total es de: ";
    final public static String NO_AUTORIZED = "No está autorizado para ingresar";
    final public static String INCOMPLETED_INFORMATION = "Información incompleta";
    final public static String SUCCESSFULL_LOAD_DATA_BASE = "Carga de base de datos exitosa";


    //Transacciones
    final public static int SET_CAR = 1;
    final public static int SET_MOTORCYCLE = 2;
    final public static int SET_COST = 3;
    final public static int SET_MOTORCYCLE_NO_AUTORIZED = 4;
    final public static int CHECKOUT_CAR_MOTORCYCLE = 5;


}
